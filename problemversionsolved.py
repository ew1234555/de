...
class Solver():
    def __init__(self):
        self.euler = Euler()
        self.ie = ImprovedEuler()
        self.rk = RungeKuta()

        self.f = lambda x, y: y ** 2 * math.exp(x) - 2 * y;
        # solution of y' = f(x,y) has a vertical asymptote in  x = 1.458675145387081
        self.x0 = 1
        self.y0 = 1
...
class MyDynamicMplCanvas(SuperCanvas):
    ...
    def update_figure(self):
        ...
        if float(self.solver.X) > float(1.458675145387081) and float(self.solver.x0) > float(1.458675145387081):
            ...
            self.solver.euler.solve(self.solver.f, self.solver.x0, self.solver.y0, 1.45867514538708, 10)

            self.axes.plot(self.solver.euler.X, self.solver.euler.Y, 'r')

            self.solver.euler.solve(self.solver.f, 2, -0.18841529588316633, self.solver.X, 1000)
